 ...
 * @version 1.4.0
 *
 * @update 2015-03-11 董经伟
 * @update 2015-03-31 董经伟
 *   1.重构, 优化
 *   2.将 history 分离
 *   3.去除 data-page, 所有页面均加入历史
 * @update 2015-06-09 董经伟
 *   1.抽离 triggerPageOpen 和 preventPageOpen 方法
 *   2.新增 setPageState 方法和 $.ui.page.setState 接口
 * @update 2015-09-16 董经伟
 *   1.新增 hold-data, 可保留在浏览器历史中不被清除
 * @update 2015-12-09 董经伟
 *   1.修复微信端不能改Title的Bug
 * @update 2016-05-17 董经伟
 *   1.HACK: fixed first time skip history
 * @update 2016-06-03 董经伟
 *   1.Bugfix: hold-data should add history
 * @update 2016-07-07 董经伟
 ...
 * @extension $.ui
 */
$.ui.page = (function (window, document, $) {
  ...
  function HANDLER_hashChange (event) {
    ...
    var pageData = $.history.data || JSON.parse(hashData || holdData || null);
    ...
    // 触发 close 事件
    var closeEvent = triggerPageClose(nowID, pageID, pageData);

    // 触发 open 事件
    var openEvent = {};
    if (!closeEvent.defaultPrevented) {
      openEvent = triggerPageOpen(pageID, nowID, pageData);
    }
    ...
    if (!openEvent.skipHistory) {
      ...
      // HACK: fixed first time skip history
  ...
  if (needFixedTitle) {
    ...
    iframe.src = "/Public/Common/index.html";
  ...
  // 触发页面 open 事件
  function triggerPageOpen (pageID, nowID, pageData) {
    /**
     * @event Page#open
  ...
  // 触发页面 close 事件
  function triggerPageClose (nowID, pageID, pageData) {
    /**
     * @event Page#close
     */
  ...
  return {
    ...
    // 打开页面
    open: function (pageID, pageData, isHoldData) {
      if (isHoldData) {
    ...
    // 打开页面(覆盖当前历史)
    replace: function (pageID, pageData, isHoldData) {
      if (isHoldData) {
    ...
    // 后退页面
    back: function (pageData) {
      $.history.data = pageData;
      window.history.back();
    },

    // 设置文档标题 (微信)
    setTitle: function (pageID, title){
      var page = $(pageID)[0];
      if (page) {
        if (needFixedTitle) {
    ...
    // 获取页面状态
    getState: function (pageID, stateID) {
      var page = $(pageID)[0];
      if (!page) {
        return;
      }
    ...
    // 设置页面状态
    setState: function (pageID, stateID) {
      var page = $(pageID)[0];
      if (!page) {
        return;
      }
